...
仅文档变更只同步 `source`，不构建、切换 `current` 或重启。2026-09-15 最新发布：应用 `e0177d42399662a1824c848cf27853e196705303`（1.4.4，常驻存档提醒），先通过本地 58 项测试、构建及相关独立档浏览器验收，再经 Mini Node 22.23.1 下 54 项非原画归档测试/构建、59 文件与本地 SHA-256 一致后原子切换。旧 `d0287e31c04b3671188911b33548a94f8dddbb04` 及更早版本保留回滚；无需重启，无数据库、上传服务或环境变量迁移。HTTPS 资源/TLS 与 HTTP 旧入口通过，正式内置浏览器仍连接关闭，交互冒烟待补；未读取 Token 或操作正常档。

发布门槛：本地自动化与构建、相关独立档浏览器交互均通过后，才切正式 `current`；本地浏览器受阻先暂停发布。发布后的正式 HTTPS 冒烟独立记录，不以资源 200 或旧版结果替代交互验收。
